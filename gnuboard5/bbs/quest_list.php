...
$qstr = get_quest_queryStr($_GET,$qstr);
// 분류 사용 여부
$is_category = false;
$category_option = '';
if ($board['bo_use_category']) {
    $is_category = true;
    $category_href = G5_BBS_URL.'/quest.php?bo_table='.$bo_table;
...
if ( $quest) {     //검색이면
    ...
    $sql_search .= get_quest_searchStr($_GET,'');
...
if ($is_search_bbs) {
    ...
    if($bo_table === 'quest'){ // quest 게시판일 경우
        ...
        $sql_search .= get_quest_searchStr($_GET,$sql_search);      
        $sql = " select * 
                 from {$write_table}
                 join wow_db.quest_template
                 on  {$write_table}.wr_id = wow_db.quest_template.entry     
...
$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './quest.php?bo_table='.$bo_table.$qstr.'&amp;page=');

$list_href = '';
$prev_part_href = '';
$next_part_href = '';
if ($is_search_bbs) {
    $list_href = './quest.php?bo_table='.$bo_table;
    ...
    if (isset($min_spt) && $prev_spt >= $min_spt) {
        ...
        $prev_part_href = './quest.php?bo_table='.$bo_table.$qstr1.'&amp;spt='.$prev_spt.'&amp;page=1';
    ...
    if ($next_spt < 0) {
        $qstr1 = preg_replace($patterns, '', $qstr);
        $next_part_href = './quest.php?bo_table='.$bo_table.$qstr1.'&amp;spt='.$next_spt.'&amp;page=1';
